    ...
    if (is_from_restart) {
        ...
        const double zeta = 10.79586*(1-(273.16/p_T0))-5.02808
                        ...
                        *(1-pow(10,(-8.29692*((p_T0/273.16)-1))))
                        -(4.2873e-4)*(1-pow(10,(-4.76955*((273.16/p_T0)-1))))
                        -2.2195983;
        ...
        if (p_src_type_int == LIGHTNING)
            readLightningInput("lightning.txt");
    }
}
                                           ...
                                           const double time) const
{
   ...
   TBOX_ASSERT(patch_geom);   
    ...
    if (p_use_sample_rate_dt == 1)
        dt = 1.0/p_wav_sample_rate;
        ...
        if (p_bc_type_L_int == ABSORB) {
            ...
            for (int j = ifirst(1); j <= ilast(1); j++) {
                ...
                for (int i = ifirst(0); (fabs(1.0-tmp) > ep) && (i <= ilast(0)); i++) {
                    ...
                    tmp = -exp(-log(2.0)/SQUARE(p_bc_absorb_width)*SQUARE(xc[0]-gridXLo[0]))+1;
                    idx2d =
                        (j-ifirst(1)+ghost_cells(1))*patch_width+(i-ifirst(0)+ghost_cells(0));
                    ...
                    w_n_data[4*patch_area + idx2d] =
                        (w_n_data[4*patch_area + idx2d]-p_T0*p_rho0)*tmp+p_T0*p_rho0;
                                                ...
                                                const hier::IntVector& ghost_width_to_fill)
{
   ...
   boost::shared_ptr<pdat::CellData<double> > source_grid(
    ...
    const hier::IntVector ghost_cells = w_n->getGhostCellWidth();
    const int patch_width = (ilastP(0)+1)-ifirstP(0)+2*ghost_cells(0);
        ...
        if (bdry_loc == 0) {
            for (int j = ifirst(1); j <= ilast(1); j++) {
                for (int i = ilast(0); i >= ifirst(0); i--) {
                    int idx2d    =
                        (j-ifirstP(1)+ghost_cells(1))*patch_width + (i  -ifirstP(0)+ghost_cells(0));
                    int idx2dIP1 =
                        (j-ifirstP(1)+ghost_cells(1))*patch_width + (i+1-ifirstP(0)+ghost_cells(0));
                    ...
                    for (int k = 0; k < NEQU; k++) {
                        ...
                        if ((k == 1) && (i == ilast(0)) && (p_bc_type_L_int == REFLECT))
                            w_n_data[idx3d] = -w_n_data[idx3d];
                    }
                }
            }
        }
            ...
            for (int j = ifirst(1); j <= ilast(1); j++) {
                for (int i = ifirst(0); i <= ilast(0); i++) {
                    ...
                    for (int k = 0; k < NEQU; k++) {
                        ...
                        if ((k == 1) && (i == ifirst(0)) && (p_bc_type_R_int == REFLECT))
                            w_n_data[idx3d] = -w_n_data[idx3d];
                    }
                }
            }
            ...
            for (int j = ilast(1); j >= ifirst(1); j--) {
                for (int i = ilast(0); i >= ifirst(0); i--) {
                    int idx2d    =
                        (j  -ifirstP(1)+ghost_cells(1))*patch_width + (i-ifirstP(0)+ghost_cells(0));
                    int idx2dJP1 =
                        (j+1-ifirstP(1)+ghost_cells(1))*patch_width + (i-ifirstP(0)+ghost_cells(0));
                    ...
                    for (int k = 0; k < NEQU; k++) {
                        ...
                        if ((k == 2) && (j == ilast(1)) && (p_bc_type_B_int == REFLECT))
                            w_n_data[idx3d] = -w_n_data[idx3d];
                    }
                }
            }
            ...
            for (int j = ifirst(1); j <= ilast(1); j++) {
                for (int i = ifirst(0); i <= ilast(0); i++) {
                    ...
                    for (int k = 0; k < NEQU; k++) {
                        ...
                        if ((k == 2) && (j == ifirst(1)) && (p_bc_type_T_int == REFLECT))
                            w_n_data[idx3d] = -w_n_data[idx3d];
                    }
                }
            }
...
// Print information about all variables and constants, etc. used in the simulation.
void enlightning::printClassData(ostream& os) const
{
    ...
    os << "tag_tolerance: " << p_tag_tolerance[0] << ", " << p_tag_tolerance[1] << endl;
...
void enlightning::getFromRestart()
{
    ...
    for (int i = 0; i < p_dim.getValue(); i++) {
        if (p_nghosts(i) != CELLG) {
            TBOX_ERROR(p_object_name << ": "
                       << "Key data `p_nghosts' in restart file != CELLG." << endl);
        }
    }
    ...
    for (int i = 0; i < p_dim.getValue(); i++) {
        if (p_zero_ghosts(i) != 0) {
            TBOX_ERROR(p_object_name << ": "
                       << "Key data `p_zero_ghosts' in restart file != 0." << endl);
        }
    }
...
int enlightning::shouldAddSource()
{
    ...
    if ((p_src_mode_int == DRIVEN) && (p_iteration_number >= p_src_pulse_times[0])) {
        add_source = 1;
    } else if (p_src_mode_int == PULSE) {
        for (int i = 0; i < p_src_num_pulses; i++) {
            // p_iteration_number+1 because this is called before main() increments it.
                    ...
                    for (int j = ifirst(1); j <= ilast(1); j++) {
                        ...
                        for (int i = ifirst(0); i <= ilast(0); i++) {
                            ...
                            source_data[idx] = p_src_amp
                                *exp(-log(2.0)/SQUARE(p_src_spread)*(x0*x0+x1*x1));
                        }
                    }
                }
                
                // Draw a sine wave shaped pulse (used before lightning channel capability was added).
                if (p_src_type_int == SINE) {
                    for (int j = ifirst(1); j <= ilast(1); j++) {
                        ...
                        for (int i = ifirst(0); i <= ilast(0); i++) {
                            ...
                            idx = (j-ifirst(1))*patch_width+(i-ifirst(0));
                            source_data[idx] = p_src_amp
                                *exp(-log(2.0)/SQUARE(p_src_spread)*SQUARE((x0)
                                -p_src_sine_amp*sin(p_src_sine_freq*(x1))));
                        }
                    }
                }
                
                // Draw a lightning channel source using coordinates already read from file.
                if (p_src_type_int == LIGHTNING) {
                    for (int k = 0; k < p_lightning_input_line_count; k++) {
                        int orientation = 0;
                        double theta=-atan((p_lightning_coords[k*4+3]-p_lightning_coords[k*4+1])
                                           /(p_lightning_coords[k*4+2]-p_lightning_coords[k*4+0]));
                        if (((p_lightning_coords[k*4+0]>p_lightning_coords[k*4+2])
                             && (p_lightning_coords[k*4+1]>p_lightning_coords[k*4+3]))
                            || ((p_lightning_coords[k*4+0]<=p_lightning_coords[k*4+2])
                             && (p_lightning_coords[k*4+1]>=p_lightning_coords[k*4+3])))
                            orientation = 1;
                        else if (((p_lightning_coords[k*4+0]>=p_lightning_coords[k*4+2])
                                  && (p_lightning_coords[k*4+1]<=p_lightning_coords[k*4+3]))
                                 || ((p_lightning_coords[k*4+0]<p_lightning_coords[k*4+2])
                                  && (p_lightning_coords[k*4+1]<p_lightning_coords[k*4+3])))
                                 orientation = 2;
                        ...
                        for (int j = ifirst(1); j <= ilast(1); j++) {
                            ...
                            for (int i = ifirst(0); i <= ilast(0); i++) {
                                ...
                                double line0 = tan(PI/2.0-theta)
                                    *(xc[0]-p_lightning_coords[k*4+0])+p_lightning_coords[k*4+1];
                                double line1 = tan(PI/2.0-theta)
                                    *(xc[0]-p_lightning_coords[k*4+2])+p_lightning_coords[k*4+3];
                                idx = (j-ifirst(1))*patch_width+(i-ifirst(0));
                                if ((orientation==1 && xc[1]>=line0)
                                    ...
                                    source_data[idx] = source_data[idx]+temp-(source_data[idx]/p_src_amp)*temp;
                                }
                                ...
                                if ((orientation==1 && xc[1]<line0 && xc[1]>line1)
                                    || (orientation==2 && xc[1]>line0 && xc[1]<line1)) {
                                    double temp = p_src_amp*exp(-log(2.0)/SQUARE(p_src_spread)
                ...
                if (p_src_type_int == POINT) {
                    for (int j = ifirst(1); j <= ilast(1); j++) {
                        ...
                        for (int i = ifirst(0); i <= ilast(0); i++) {
                            ...
                            source_data[idx] = p_src_amp
                                *exp(-log(2.0)/SQUARE(p_src_spread)*(x0*x0+x1*x1))
                                *sin(p_src_omega*(p_iteration_number+1-p_src_pulse_times[0])*dt);
                        }
                    }
                }
                if (p_src_type_int == SINE || p_src_type_int == LIGHTNING)
                    TBOX_ERROR("DRIVEN SINE or LIGHTNING source mode not implemented" << endl);
            }
        }
    }
    t_addsource->stop();
...
// If source is a lightning channel, read in the geometry of the channel from file.
        ...
        for (hier::PatchLevel::iterator patch_iterator(level->begin());
            ...
            for (int k = 0; k < p_num_mics; k++) {
                ...
                if ((p_mic_pos_x[k] >= patchXLo[0]) && (p_mic_pos_x[k] <= patchXHi[0])
                    && (p_mic_pos_y[k] >= patchXLo[1]) && (p_mic_pos_y[k] <= patchXHi[1])) {
                    FILE *fw1, *fw2;
                    char filename1[64];
                    char filename2[64];
		        ...
		        BOOST_CAST<pdat::CellData<double>, hier::PatchData>(
                    ...
                    //cout << "i: " << i << " j: " << j << endl;
                    //cout << "ifirst[0]: " << ifirst[0] << " ifirst[1]: " << ifirst[1] << endl;
                    //cout << "ilast[0]: " << ilast[0] << " ilast[1]: " << ilast[1] << endl;
                    //cout << "patchXLo[0]: " << patchXLo[0] << " patchXLo[1]: " << patchXLo[1] << endl;
                    //cout << "patchXHi[0]: " << patchXHi[0] << " patchXHi[1]: " << patchXHi[1] << endl;
